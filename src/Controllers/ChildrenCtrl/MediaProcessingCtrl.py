...
class MediaProcessingCtrl(BaseCtrl):
    ...
    _support_img_ext = ['bmp', 'dng', 'jpeg', 'jpg', 'mpo', 'png', 'tif', 'tiff', 'webp', 'pfm', 'JPG'] 
    _support_vid_ext = ['asf', 'avi', 'gif', 'm4v', 'mkv', 'mov', 'mp4', 'mpeg', 'mpg', 'ts', 'wmv', 'webm']
    _support_model_ext = ['torchscript', 'pt', 'onnx', 'engine', 'mlmodel', 'pb', 'tflite']
    #Рандомные улицы
    _street = ['Ushakova', 'Kirovogradskaia', 'Naximova', 'Zakamskaia', 'Ribalko', 'Astraxanskaia']

    #Основной процесс обработки видео, записи стопкадров в папку Media/{nametable} и координат в таблицу
    def _videoProcessing(self, model, video_path, database, nametable, is_save_frame):
        # if video_path.split('.')[-1] in self._support_vid_ext:
    ...
    def _imageProcessing(self, model, image_path, database, nametable, is_save_frame):
        # if image_path.split('.')[-1] in self._support_img_ext:
